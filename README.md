...
TradingAgents is a state-of-the-art investment decision-making dashboard powered by **FastAPI** and **LangGraph**. It orchestrates a multi-agent AI system where specialized agents (analysts, researchers, risk debaters, and portfolio managers) collaborate, debate, and verify information; a deterministic stability controller then records or accepts the canonical decision before application-side execution controls run.
...
*   **Multi-Agent Decision Workflows:** A state-machine structured via LangGraph where a neutral planner guides independent research, analysts fetch data, bull/bear researchers debate theses, risk agents surface guardrails, the Portfolio Manager proposes a trade, and a deterministic controller validates material decision changes.
...
*   **Backend:** Python 3.11–3.13, FastAPI, SQLAlchemy (Asyncio / asyncpg), PostgreSQL, APScheduler, LangGraph, LangChain Core. Optional Redis + arq for horizontal scaling.
...
│   │   ├── strategy_continuity.md # Persistent thesis + stability controls
