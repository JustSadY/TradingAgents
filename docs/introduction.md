...
3.  **Asynchronous Responsiveness:** The web frontend (React SPA) and backend (FastAPI) communicate continuously using WebSockets. Long-running multi-agent tasks run as asynchronous background tasks — or, when Redis is configured (`ANALYSIS_QUEUE_MODE=worker`), in a dedicated arq worker process — to keep the system responsive.
