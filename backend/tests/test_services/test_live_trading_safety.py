...
async def test_live_alpaca_executor_rejects_before_constructing_sdk_clients(monkeypatch):
    ...
    async def unexpected_clients(*_args, **_kwargs):
        raise AssertionError("disabled live trading must not construct Alpaca SDK clients")

    trader = alpaca.AlpacaTrader(db=object(), mode="live")
    monkeypatch.setattr(trader, "_clients", unexpected_clients)
