...
async def test_worker_treats_cancelled_analysis_as_terminal(monkeypatch):
    """The worker must terminalize cancellation through its public cleanup boundary."""
    ...
    terminal: list[tuple[str, int | None, str, str, bool]] = []

    async def terminalize(task_id, user_id, *, status, message, persist_analysis_row):
        terminal.append((task_id, user_id, status, message, persist_analysis_row))
    ...
    monkeypatch.setattr(worker, "_terminalize_worker_failure", terminalize)

    with pytest.raises(asyncio.CancelledError):
        await worker.run_analysis_job({}, "AAPL", "2026-07-26", "stock", None, "worker-stop")

    assert terminal == [("worker-stop", None, "cancelled", "Analysis cancelled.", True)]
