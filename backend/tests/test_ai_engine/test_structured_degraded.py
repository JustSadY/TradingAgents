...
class _MalformedStructuredLLM:
    ...
    async def ainvoke(self, _prompt):
        self.calls += 1
        # This used to escape the helper as a raw AIMessage-like object. The
        # caller then attempted ``result.rating`` / ``result.research_bias``
        # and failed outside the structured recovery boundary.
