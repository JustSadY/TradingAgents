...
class TestPersonaRepository:
    async def test_create_user_persona(self, db: AsyncSession, test_user):
        persona = await create_user_persona(
            ...
            description="Low risk tolerance", instructions="Be careful",
        )
    ...
    async def test_list_user_personas(self, db: AsyncSession, test_user):
        await create_user_persona(db, user_id=test_user.id, key="p1", label="P1",
                                  description="", instructions="")
        await create_user_persona(db, user_id=test_user.id, key="p2", label="P2",
                                  description="", instructions="")
    ...
    async def test_get_user_persona_by_key(self, db: AsyncSession, test_user):
        await create_user_persona(db, user_id=test_user.id, key="aggressive", label="Aggressive",
                                  description="High risk", instructions="Take risks")
    ...
    async def test_update_user_persona(self, db: AsyncSession, test_user):
        persona = await create_user_persona(db, user_id=test_user.id, key="balanced", label="Balanced",
                                            description="Middle ground", instructions="Be balanced")
    ...
    async def test_delete_user_persona(self, db: AsyncSession, test_user):
        persona = await create_user_persona(db, user_id=test_user.id, key="temp", label="Temp",
                                            description="", instructions="")
