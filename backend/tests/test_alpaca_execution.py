...
@pytest.mark.asyncio
async def test_traders_connection_close_and_context_manager():
    from backend.services.execution.simulation import SimulationTrader

    # Case 1: db is not None (external db session)
    db_mock = AsyncMock()
    async with AlpacaTrader(db=db_mock) as trader:
        assert trader._is_local_db is False
    # should NOT call close on db since it is not local
    db_mock.close.assert_not_called()

    async with SimulationTrader(db=db_mock) as trader:
        assert trader._is_local_db is False
    db_mock.close.assert_not_called()

    # Case 2: db is None (local db session)
    # Mock AsyncSessionLocal in alpaca and simulation to return a mock session
    db_local_mock = AsyncMock()
    with patch("backend.services.execution.alpaca.AsyncSessionLocal", return_value=db_local_mock):
        async with AlpacaTrader() as trader:
            assert trader._is_local_db is True
        db_local_mock.close.assert_called_once()

    db_local_mock_sim = AsyncMock()
    with patch("backend.services.execution.simulation.AsyncSessionLocal", return_value=db_local_mock_sim):
        async with SimulationTrader() as trader:
            assert trader._is_local_db is True
        db_local_mock_sim.close.assert_called_once()
