...
async def chat(db: AsyncSession, user: User, message: str) -> dict:
    ...
    if user.is_admin:
        allowed_pages: set[str] = {"analysis", "trading", "portfolio", "alerts", "watchlist"}
    else:
        allowed_pages = await list_allowed_page_keys(db, user.id)
    ...
    try:
        client = create_llm_client(provider=active_provider, model=active_model, api_key=user_key)
        llm = client.get_llm()
    ...
    for _ in range(_MAX_TOOL_LOOP):
        try:
            ...
            final_content = f"I encountered an error: {e}"
            break
        ...
        final_content = "I could not complete the request within the allowed steps. Please try again."

    if not final_content:
        final_content = "I'm sorry, I could not generate a response. Please try again."
...
def _make_tools(db: AsyncSession, user: User, allowed_pages: set[str]) -> list:
    ...
    async def get_analysis_history(ticker: str = "", limit: int = 5) -> str:
        ...
        try:
            ...
            if not rows:
                filter_str = f" for {ticker.upper()}" if ticker else ""
                return f"No completed analyses found{filter_str}."
            ...
            for row in rows:
                ...
                if len(row.final_decision or "") > 300:
                    summary += "…"
                lines.append(
                    f"[ID {row.id}] {row.ticker} | {row.trade_date} | Signal: {row.signal or 'N/A'}\n  {summary}"
                )
            return "\n\n".join(lines)
        except Exception as e:
    ...
    async def run_stock_analysis(ticker: str) -> str:
        ...
        try:
            ...
            return (
                ...
                f"It will take about 2-3 minutes. Go to the Analysis page to track progress and see results."
            )
        except Exception as e:
    ...
    async def place_paper_order(ticker: str, action: str, quantity: float) -> str:
        ...
        try:
            ...
            return (
                f"Order placed: {act} {filled:.4f} shares of {ticker.upper()} @ ${float(price):,.2f}. "
                f"Total: ${float(filled) * float(price):,.2f}."
            )
