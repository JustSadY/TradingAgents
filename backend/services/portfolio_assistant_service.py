...
def _make_tools(db: AsyncSession, user: User, allowed_pages: set[str]) -> list:
    ...
    return [
        get_portfolio_summary, get_analysis_history, get_analysis_report, get_live_price,
        get_watchlist, get_alerts, create_price_alert, run_stock_analysis, place_paper_order,
    ]
...
async def _tool_get_analysis_history(db: AsyncSession, user: User, ticker: str, limit: int) -> str:
    try:
        ...
        q = (
            select(AnalysisResult.id, AnalysisResult.ticker, AnalysisResult.trade_date, AnalysisResult.signal, AnalysisResult.final_decision, AnalysisResult.created_at)
            .where(AnalysisResult.status == "completed").order_by(AnalysisResult.created_at.desc()).limit(n)
        )
...
async def _tool_get_analysis_report(db: AsyncSession, user: User, analysis_id: int) -> str:
    try:
        ...
        sections = []
        for label, attr in [("Final Decision", "final_decision"), ("Market Report", "market_report"), ("Fundamentals", "fundamentals_report"), ("News", "news_report"), ("Sentiment", "sentiment_report")]:
...
async def _tool_create_price_alert(user: User, allowed_pages: set[str], ticker: str, condition: str, target_price: float) -> str:
    ...
    try:
        ...
        async with AsyncSessionLocal() as alert_db:
            alert = await _create(alert_db, user_id=user.id, ticker=ticker.strip().upper(), condition=cond, target_price=target_price, auto_analyze=False)
...
async def _tool_place_paper_order(user: User, allowed_pages: set[str], ticker: str, action: str, quantity: float) -> str:
    ...
    try:
        ...
        async with AsyncSessionLocal() as order_db:
            result = await execute_order(order_db, ticker=ticker.strip().upper(), action=act, quantity=quantity, user=user)
