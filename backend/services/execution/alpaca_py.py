"""Official alpaca-py execution adapter.

This is preferred by the execution factory whenever ``alpaca-py`` is installed.
The legacy HTTP adapter remains only as a compatibility fallback so existing
locked deployments do not fail to boot before the optional SDK is installed.
"""
...
class AlpacaPyTrader(BaseTraderInterface):
