...
def create_analyst_ratings_analyst(llm):
    ...
    async def analyst_ratings_node(state):
        ...
        try:
            ...
            _logger.warning("Data fetch failed in analyst_ratings_analyst: %s", _e)
        ...
        system_message = """You are a sell-side ratings analyst. Your role is EXCLUSIVELY consensus measurement — you do NOT make buy/sell/hold recommendations. Output only ratings-consensus observations.
...
2. **Consensus Direction:** Judge the net tilt (bullish, neutral, bearish) and whether it is strengthening or weakening across recent periods. Assign a confidence level.
...
- **IMPORTANT:** NEVER output a Buy/Sell/Hold rating. Ratings consensus analysis only.
- Assign a confidence level (HIGH/MEDIUM/LOW) to each key observation.

### Output Format:
Your final report MUST follow this structure:
1. **Executive Summary:** A 3-bullet summary of the consensus (direction, implied upside, level of agreement), each with confidence.
